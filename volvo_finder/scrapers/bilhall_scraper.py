"""Bilhall.se scraper using BeautifulSoup4 HTML parsing."""
import json
import re
...
_SEARCH_URL = "https://www.bilhall.se/begagnade-bilar/"
_SEARCH_PARAMS = {
    "make": "Volvo",
    "models[]": ["V60", "V90", "XC60"],
}
    ...
    def parse(self, raw_data: list[dict]) -> list[dict]:
        ...
        for item in raw_data:
            try:
                ...
                cards = (
                    soup.select("article.car-card")
                    or soup.select("div.car-card")
                    or soup.select("[class*='car-item']")
                    or soup.select("article[class*='vehicle']")
                    or soup.select("li.vehicle-item")
                )
