"""KVD Bil scraper using Playwright for JS-rendered search results."""
import json
import re
import urllib.parse
...
class KvdbilScraper(BaseScraper):
    ...
    async def fetch(self) -> list[dict]:
        """Fetch KVD listings using headless browser (page is JS-rendered)."""
        all_items: list[dict] = []
        ...
        for model in _SEARCH_MODELS:
            ...
            while page <= 10:
                ...
                html = cached
                if not html:
                    self.logger.info(json.dumps({"event": "kvd_browser_fetch", "model": model, "page": page}))
                ...
                cards = (
                    ...
                    or soup.select("[class*='ObjectCard']")
                ...
                if not cards:
                    self.logger.info(json.dumps({
                        "event": "kvd_no_cards", "model": model, "page": page,
                        "html_snippet": soup.body.get_text(" ", strip=True)[:400] if soup.body else "",
                    }))
                    break

                self.logger.info(json.dumps({
                    "event": "kvd_cards_found", "model": model, "page": page, "count": len(cards),
                }))
                for card in cards:
    ...
    def parse(self, raw_data: list[dict]) -> list[dict]:
        ...
        for item in raw_data:
            try:
                ...
                title_el = soup.select_one("h2") or soup.select_one("[class*='title']")
                ...
                year = 0
                year_el = soup.select_one("[class*='year']") or soup.select_one("[class*='år']")
                if year_el:
